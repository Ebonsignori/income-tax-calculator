...
- [Dependency Management](#dependency-management)
  - [update-dependencies.ts](#update-dependenciests)
...
**TypeScript script** that intelligently updates npm dependencies using GitHub Copilot CLI.

**Usage:**
```bash
npm run update-dependencies              # Full update process
npm run update-dependencies -- --dry-run         # Preview updates
npm run update-dependencies -- --group-only      # Show package groupings
npm run update-dependencies -- --skip-types      # Skip @types updates
```

**Options:**
- `--dry-run` - Show what would be updated without making changes
- `--group-only` - Display package groupings and exit
- `--skip-types` - Skip automatic @types package updates
- `--help` - Show help message

**Package Grouping:**

The script intelligently groups related packages to ensure compatibility:

| Group | Packages | Purpose |
|-------|----------|---------|
| `@types` | All `@types/*` packages | TypeScript type definitions - auto-updated together |
| `next` | next, next-sitemap, next-pwa, react, react-dom | Ensures React version compatibility with Next.js |
| `typescript` | ts-node, tsconfig-paths, typescript | Ensures TypeScript tooling compatibility |
| `eslint` | eslint*, prettier | Ensures linter/formatter compatibility |
| `@mui` | All `@mui/*` packages | Material-UI components - must be same version |
| `@emotion` | All `@emotion/*` packages | Styling library - must be same version |
| `@playwright` | All `@playwright/*` packages | E2E testing - must be same version |
| `@typescript-eslint` | @typescript-eslint/* | ESLint TypeScript support |
| `@vitest` | All `@vitest/*` packages | Unit testing framework |
| `@fontsource` | All `@fontsource/*` packages | Font packages |
| Standalone | Individual packages | Updated one at a time |

**How it works:**

1. **Grouping** - Related packages are grouped to maintain compatibility
2. **@types Updates** - Type definitions are updated directly (no code changes needed)
3. **Peer Dependency Detection** - Detects version constraints within groups
4. **Copilot CLI Integration** - For other groups, uses Copilot to:
   - Research breaking changes
   - Update to compatible versions (not always latest if peer deps conflict)
   - Make necessary code modifications
   - Update imports and usages
5. **Validation** - After each group update, runs:
   - `npm run lint`
   - `npm test` (unit + e2e)
   - `npm run build`
6. **Logging** - Creates detailed logs in `logs/update-deps-{TIMESTAMP}.log`

**When to use:**
- Regular maintenance (monthly/quarterly)
- Before major releases
- When dependencies have security updates
- To adopt new framework features
...
# Show package groupings
npm run update-dependencies -- --group-only
