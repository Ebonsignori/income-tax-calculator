...
export default {
  // Kentucky publishes ONE standard deduction that every filing status uses; it
  // is not doubled on a joint return. Form 740, Line 10: "Nonitemizers, enter
  // the standard deduction of $2,980. If married filing separately on a combined
  // return, enter $2,980 in both Columns A and B. If filing a joint return, only
  // one $2,980 standard deduction is allowed." The per-spouse doubling belongs to
  ...
  // Verified 2026-09-07: correct as written, including the 1.00x married
  // ratio that reads as a bug next to 2024-2026.
  // Source: 2023 Form 740 instructions, "Line 10, Deductions", p.9.
  // https://revenue.ky.gov/Forms/740%20Packet%20Instructions%202023.pdf
  [STANDARD_DEDUCTION]: {
    [SINGLE]: 2980,
    [MARRIED]: 2980,
  ...
  // every filing status.
  // Verified 2026-09-07. Source: 2023 Form 740 instructions, "Tax
  // Computation - Multiply line 11 by four and one-half percent (.045)".
  // https://revenue.ky.gov/Forms/740%20Packet%20Instructions%202023.pdf
  [STATE_INCOME]: {
    [SINGLE]: [
      {
  ...
  [CITIES]: {
    ...
    // Corrected 2026-09-07: was 1.85, the city component alone. RESIDENT
    // TOTAL, two components levied and collected separately:
    //   1.85%  City of Bowling Green (raised to 2.0% on 2024-01-01)
    ...
    [BOWLING_GREEN]: {
      [OCCUPATIONAL_TAX]: {
        [SINGLE]: [{ min: 0, max: INFINITY, rate: 2.35 }],
        [MARRIED]: [{ min: 0, max: INFINITY, rate: 2.35 }],
        [MARRIED_SEPARATELY]: [{ min: 0, max: INFINITY, rate: 2.35 }],
        [HEAD_OF_HOUSEHOLD]: [{ min: 0, max: INFINITY, rate: 2.35 }],
      },
    },
    // Verified 2026-09-07: 2.45% is the PAYROLL rate. Covington's Finance
    // page states both figures, and they differ: "Employers in Covington are
    // required to withhold 2.45% on all compensation paid to their employees",
    // while "The Occupational License Tax is assessed at 2.5% of the net
    // profits". 2.45% is unchanged in snapshots from 2023-06 and 2025-06 and
    // on the live page today.
    ...
    // Security wage base ($160,200 for 2023) are not taxed - the trailing 0%
    ...
    [NEWPORT]: {
      [OCCUPATIONAL_TAX]: {
        [SINGLE]: [
          { min: 0, max: 160200, rate: 2.5 },
          { min: 160200, max: INFINITY, rate: 0 },
        ],
        [MARRIED]: [
          { min: 0, max: 160200, rate: 2.5 },
          { min: 160200, max: INFINITY, rate: 0 },
    ...
    //          wage base ($160,200 for 2023)
    //   0.80%  Boone County Fiscal Court, ord. 07-27, capped at $72,906
    //          of wages for 2023 (maximum payment $583.25)
    ...
    //   3.30% to   $72,906   mental health has capped out
    //   2.50% to  $160,200   the county fiscal court fee has too
    ...
    [FLORENCE]: {
      [OCCUPATIONAL_TAX]: {
        [SINGLE]: [
          ...
          { min: 16666, max: 72906, rate: 3.3 },
          { min: 72906, max: 160200, rate: 2.5 },
          { min: 160200, max: INFINITY, rate: 0.5 },
        ],
        [MARRIED]: [
          { min: 0, max: 16666, rate: 3.45 },
          { min: 16666, max: 72906, rate: 3.3 },
    ...
    // Verified 2026-09-07: 1.35% is correct for 2023. Ordinance No. 13-2023,
    // passed 2023-06-26, raised it to 1.95% effective 2024-01-01; the city
    // notes the rate "had not been changed since 2006".
